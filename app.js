(() => {
  ...
  // A4 세로 인쇄는 1페이지 요약을 우선하기 위해 상세 드롭다운을 잠시 접습니다.
  let printDetailState = [];

  window.addEventListener("beforeprint", () => {
    const details = [...document.querySelectorAll("details.detail-card")];
    printDetailState = details.map(detail => detail.open);
  ...
  window.addEventListener("afterprint", () => {
    ...
    details.forEach((detail, index) => {
      detail.open = Boolean(printDetailState[index]);
    });
    printDetailState = [];
  });
