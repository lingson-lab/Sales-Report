(() => {
  ...
  const FIELD_ALIASES = {
    date: ["수강시작일", "수강 시작일", "수강시작일자", "수강 시작일자"],
  ...
  function getComparisonRows(month) {
    const prevKey = previousMonthKey(month);
    const prevRows = state.rows.filter(row => row.__month === prevKey);
    ...
    return {
      ...
      available: true,
      samePeriod: false,
      label: `수강시작일 기준 전월 대비 · ${formatMonth(prevKey)} 전체`
    };
  }
  ...
  function aggregateFacilities(rows) {
    ...
    rows.forEach(row => {
      // 취소/기타 거래는 실매출 집계에서 제외
      if (!["sale", "refund"].includes(row.__transactionType)) return;
  ...
  function renderFacility(currentRows, previousRows, comparisonAvailable) {
    ...
    const allMembers = new Set(
      currentRows
        .filter(row => row.__transactionType === "sale")
        .map(row =>
          state.keys.member
            ? String(row[state.keys.member] ?? "").trim()
            : ""
  ...
  function render(month) {
    ...
    const comparison = getComparisonRows(month);
    ...
    if (dates.length) {
      els.periodText.textContent =
        `${formatDate(dates[0])} ~ ${formatDate(dates[dates.length - 1])} · 수강시작일 기준월 ${formatMonth(month)}`;
    }
  ...
  async function loadFile(file) {
    try {
      ...
      if (unknownTypes.length) {
        setStatus(
          `${file.name} · ${state.rows.length.toLocaleString("ko-KR")}행 분석 완료 · 미인식 거래종류는 실매출에서 제외: ${unknownTypes.join(", ")}`,
          "warn"
        );
      } else {
        setStatus(
          `${file.name} · ${state.rows.length.toLocaleString("ko-KR")}행 분석 완료 · 수강시작일 기준 실매출 = 매출 − 환불, 취소 제외`,
          "ok"
        );
      }
    } catch (err) {
